  ...
  <script type="text/javascript">
    $(document).ready(function(){
	  $("#delete").click(function() {	
		  many_deleterow("deleteMeeting.action?seleteIDs=");
	  });	
	  ...
	  var mygrid = jQuery("#grid").jqGrid({
			...
			height: "auto",
		   	colNames:['<s:text name="entity.id.label" />','<s:text name="meeting.subject.label" />',
		  		   	'<s:text name="meeting.status.label" />','<s:text name="meeting.start_date.label" />',
		  		   	'<s:text name="meeting.end_date.label" />','<s:text name="meeting.location.label" />',
		     ...
		     <span style="white-space:nowrap;">
		       <a href="javascript:void(0)" id="mtmt" class="easyui-menubutton" data-options="menu:'#mtm1',iconCls:'icon-more'"><s:text name='menu.toolbar.more.title'/></a>
				  ...
				  <s:if test="#request.user.create_meeting == 1 || #request.user.update_meeting == 1">
					<div data-options="iconCls:'icon-import'" onClick="openwindow('/crm/uploadMeeting.jsp','<s:text name="title.import.meeting" />')"><s:text name='menu.item.import.title'/></div>
				  </s:if>	  
