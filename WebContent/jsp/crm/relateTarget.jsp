...
<script type="text/javascript">	  
    $(document).ready(function(){
        var entityName = '<%=(String)request.getAttribute("entityName")%>';
    	  $("#add").click(function() {
    		  openPage('/crm/editTarget.action?relationKey=' + entityName + '&relationValue=<s:property value="id" />');
     	  });
     ...
     $('#tt').datagrid({
		...
		columns:[[
					...
					{field:'name',title:'<s:text name="entity.name.label" />',width:80,align:'center',sortable:'true',formatter:function(value,row,index){  
						   new_format_value = "<a href='editTarget.action?id=" + row.id + "' target='mainFrame'>" + value + "</a>";
